...
class _tally {
                ...
                if (this.config.sync == 'incremental') {
                    ...
                    if (/^(mssql|mysql|postgres)$/g.test(database.config.technology)) {
                        ...
                        for (let i = 0; i < lstTables.length; i++) {
                            ...
                            if (lines.length > 0) {
                                // Get header line and clean it - remove all quotes
                                let headerLine = lines[0].replace(/"/g, '').replace(/\t/g, ',');

                                // Process data lines
                                let dataLines = lines.slice(1).map(line => {
                                    // Convert tabs to commas and handle the data
                                    return line.split('\t').map(field => {
                                        // Remove existing quotes and re-quote if needed
    ...
    processTdlOutputManipulation(txt) {
        ...
        try {
            ...
            // Remove quotes from field values to prevent BigQuery character map issues
            retval = retval.replace(/\"([^\"]*)\"/g, '$1'); //remove quotes around values
        }
        catch (err) {
    ...
    processReport(targetTable, tableConfig, substitutions) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // Create clean column headers without any quotes
