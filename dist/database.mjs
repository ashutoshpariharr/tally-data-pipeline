...
class _database {
    ...
    async uploadGoogleBigQuery(targetTable) {
        try {
            // Use client/company-specific dataset: tallydb_<clientName>
            const client = (this.config.clientName || this.config.schema || 'client').toString().trim();
            if (!client) throw new Error('Client name not specified in configuration');
            const baseDataset = (this.config.dataset || 'tallydb').toString().trim();
            let datasetId = `${baseDataset}_${client}`.toLowerCase().replace(/[^a-z0-9_]/g, '_');
            datasetId = datasetId.replace(/^_+|_+$/g, '');
            if (!/^[a-z]/.test(datasetId)) datasetId = 'd_' + datasetId;

            // create dataset if it doesn't exist
            const [datasets] = await this.bigquery.getDatasets();
            if (!datasets.some(ds => ds.id === datasetId)) {
                await this.bigquery.createDataset(datasetId);
                logger.logMessage('Created BigQuery dataset: %s', datasetId);
            }
            const dataset = this.bigquery.dataset(datasetId);

            // Clean header in CSV to remove double quotes
            const csvPath = `./csv/${targetTable}.csv`;
            if (fs.existsSync(csvPath)) {
                let csvContent = fs.readFileSync(csvPath, 'utf-8');
                let csvLines = csvContent.split(/\r?\n/);
                if (csvLines.length > 0) {
                    csvLines[0] = csvLines[0].replace(/"/g, '');
                    fs.writeFileSync(csvPath, csvLines.join('\r\n'));
                }
            }

            // Load CSV data into the client dataset
            const [job] = await dataset.table(targetTable).load(csvPath, {
                sourceFormat: 'CSV',
                skipLeadingRows: 1,
            ...
            logger.logError('database.uploadGoogleBigQuery()', err);
            throw err;
        }
    }
